...
function renderRows(rows) {
  const body = document.getElementById("managerDepartmentHoursBody");
  if (!rows.length) {
...
async function queryManagerDepartmentHours() {
  const month = document.getElementById("accountSetSelect").value;
  const query = new URLSearchParams();
  if (month) query.set("month", month);
  const res = await fetch(`/employee/api/manager-department-hours?${query.toString()}`);
  const data = await res.json();
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("queryBtn").addEventListener("click", queryManagerDepartmentHours);
  document.getElementById("downloadBtn").addEventListener("click", () => {
    const month = document.getElementById("accountSetSelect").value;
    const query = new URLSearchParams();
    if (month) query.set("month", month);
    window.location.href = `/employee/api/manager-department-hours/export?${query.toString()}`;
  });
});
